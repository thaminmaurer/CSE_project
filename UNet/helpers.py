...
class CustomModelCheckpoint(tf.keras.callbacks.Callback):
    ...
    def __init__(self, checkpoint_path, save_interval):
    ...
    def on_batch_end(self, batch, logs=None):
        if batch % self.save_interval == 0:
            filepath = self.checkpoint_path.format(epoch=self.epoch, batch=batch)
            self.model.save_weights(filepath, overwrite=True)

    def on_epoch_end(self, epoch, logs=None):
          self.epoch = epoch + 1
    
# Rounds all groundtruths smaller than threshold to 0 and others to one
def round_groundtruth(gt_img, threshold):
    return np.where(gt_img >= threshold, 1, 0)
